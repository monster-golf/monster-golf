...
namespace MonsterGolf
{
   ...
   public class GolferList 
   {
      ...
      {
         return DB.GetDataSet("select * from users order by lastname, firstname");
      }
   }
	...
	public class Golfer
	{
      ...
      public Golfer(DataRow dr)
      {
		...
		public Golfer(int golferID)
		{
         ...
         DataSet ds = DB.GetDataSet("select * from users where userid = " + golferID.ToString());
      ...
      {
         DB.Execute("insert into users (firstname, lastname, hcpindex) values ('" +
      ...
      private DataSet DetailsForTournament()
      {
         ...
         string sql = "select tp.* from teamplayers tp inner join teams t on t.teamid = tp.teamid where tp.userid = " +
      ...
      public bool LoadGolfersScore() 
      {
         ...
         if (m_tournamentid > -1 && m_roundnumber > 0) 
         {
            ...
            string sql = "select * from tourneyscores where tournamentid = " +
      ...
      private void AddToScoringTable()
      {
         if (m_tournamentid != -1)
         {
            DataSet scores = DB.GetDataSet("select * from tourneyscores where userid = " + this.ID.ToString() +
            ...
            {
                DB.Execute("insert into tourneyscores (userid, tournamentid, roundnumber, handicap) values (" +
                   this.ID.ToString() + "," + m_tournamentid.ToString() + "," +
                   m_roundnumber.ToString() + "," + HcpIndex + ")");
            }
            else
            {
                DB.Execute("update tourneyscores set handicap =" + HcpIndex + " WHERE ID = " + scores.Tables[0].Rows[0]["ID"].ToString());
            }
         }
      }
      ...
      public void SaveScores()
      {
         if (m_tournamentid != -1)
         {
            ...
            string sql = "update tourneyscores set handicap=" + m_hcpindex.ToString() + ", ";
            for (int x = 1; x < 19; x++)
            {
            ...
            sql = "update teamplayers set TeeNumber=" + Tee.ToString();
            sql += " where userid = " + this.ID.ToString() + " and tournamentid = " + m_tournamentid.ToString();
            DB.Execute(sql);
         }
      }
      ...
      public bool Save() 
      {
         ...
         if ( m_tournamentid != -1 )
         {
            ...
            if ( ds.Tables[0].Rows.Count > 0 )
            {
               sql = "update teamplayers set handicap=" + HcpIndex.ToString() + ", TeeNumber=" + Tee.ToString() + " where ID = " + ds.Tables[0].Rows[0]["id"].ToString();
               DB.Execute(sql);
            }
            ...
            sql = "update Users set firstname='" + FirstName.Replace("'", "''") + "', lastname = '" + LastName.Replace("'", "''") + "' where UserID = " + this.ID;
            DB.Execute(sql);
         }
         return true;
      }

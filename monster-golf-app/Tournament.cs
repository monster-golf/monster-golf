...
namespace MonsterGolf
{
   public class Team 
   {
      ...
      {
         DB.Execute("update teams set flight='" + m_flight.Replace("'", "''") + "' where teamid = " + this.ID.ToString());
      }
      ...
      {
         DataSet ds = DB.GetDataSet("select UserID from teamplayers where teamid=" + this.m_id.ToString());
         if (ds != null) 
         {
            ...
            {
               DataSet golferDS = DB.GetDataSet("select * from Users where UserID = " + ds.Tables[0].Rows[x]["UserID"].ToString() + " order by lastname, firstname");
      ...
      {
         DB.Execute("delete from teamplayers where teamid = " + this.ID.ToString() +
            " and userid = " + golferID.ToString());
         m_numgolfers -= 1;
         if (m_numgolfers == 0)
         {
            DB.Execute("delete from Teams where teamid = " + this.ID.ToString());
            m_golfers = null;
         }
         else
         {
	...
	public class Tournament
	{
      ...
      {
         string[] flights = null;
         DataSet ds = DB.GetDataSet("select distinct flight from teams where tournamentid = " + this.TournamentID.ToString());
      ...
      {
         DB.Execute("delete from teams where teamid = " + teamID.ToString() + " and tournamentid=" + this.TournamentID.ToString());
         Save();
      }
      ...
      //   DB.Execute("update teams set flight='' where flight like '" + flight.Replace("'", "''") + "' and tournamentid=" +
      ...
      public void ModifyTeam(Team teamtoadd)
      {
         ...
         DataSet ds = DB.GetDataSet("select * from teams where tournamentid = " + this.TournamentID.ToString() + " and teamid = " + teamtoadd.ID.ToString());
         if ( ds == null || ds.Tables[0].Rows.Count == 0 ) 
         {
            teamtoadd.ID = -1;
            AddTeam(teamtoadd);
         ...
         if (teamtoadd.NumberOfGolfers > 0) 
         {
             ...
             DB.Execute("update teams set teamname='" + teamtoadd.Name.Replace("'", "''") + "', flight='" + 
      ...
      public void AddGolfer(int inGolferID, string firstname, string lastname, double handicap, int teamID, bool reload) 
      {
         ...
         if (!GolferInList(golferID)) 
         {
            if (golferID == -1)
            {
               DB.Execute("insert into users (firstname, lastname) value ('" +
               ...
               DB.Execute("update users set firstname = '" + firstname.Replace("'", "''") + 
         ...
         if (teamID > -1)
         {
            DataSet ds = DB.GetDataSet("select * from teamplayers where tournamentid = " + TournamentID +
               " and userid = " + golferID);
            if (ds.Tables[0].Rows.Count == 0)
               DB.Execute("insert into teamplayers (teamid, userid, tournamentid, Handicap) values (" + teamID.ToString() + ", " + golferID.ToString() + ", " + this.TournamentID.ToString() + ", " + handicap.ToString() + ")");
            else
               DB.Execute("update teamplayers set teamid = " + teamID.ToString() + ", Handicap = " + handicap.ToString() + " where userid= " + golferID + " and tournamentid = " + TournamentID.ToString());
         }
         if (reload)
            this.ReloadGolfers();
      }
      ...
      { 
         ...
         DB.Execute("insert into teams (teamname, flight, tournamentid) values ('" +
            teamtoadd.Name.Replace("'", "''") + "', '" + teamtoadd.Flight.Replace("'", "''") +
            "', " + this.TournamentID + ")");
         DataSet ds = DB.GetDataSet("select max(teamid) as teamid from teams");
      ...
      {
         DB.Execute("delete from courses where tournamentid=" + this.TournamentID.ToString() + " and courseid=" + courseid.ToString());
         Save();
      }
      ...
      {
         DataSet ds = DB.GetDataSet("select * from teams where tournamentid = " +
      ...
      {
         m_teams = DB.GetDataSet("select * from Teams where TournamentID=" + m_id.ToString());
      }

      public void ReloadGolfers()
      {
         m_golfers = DB.GetDataSet(
            "SELECT Users.firstname, Users.lastname, Users.userid, TeamPlayers.Handicap, Teams.TeamID, Teams.Flight, Users.WebId, Teams.TeamName, Round(Sum(TeamPlayers2.Handicap),2) AS TeamHcp, TeamPlayers.TeeNumber " +
            "FROM " +
            "((TeamPlayers  " +
            "INNER JOIN Teams ON TeamPlayers.TeamID = Teams.TeamID) " +
            "INNER JOIN Users ON TeamPlayers.UserID = Users.UserID) " +
            "INNER JOIN TeamPlayers AS TeamPlayers2 ON TeamPlayers2.TeamID = Teams.TeamID AND TeamPlayers2.TournamentID = Teams.TournamentID " +
            "WHERE Teams.TournamentID = " + m_id.ToString() + " " +
            "GROUP BY Users.firstname, Users.lastname, Users.userid, TeamPlayers.Handicap, Teams.TeamID, Teams.Flight, Users.WebId, Teams.TeamName, TeamPlayers.TeeNumber " +
      ...
      private void Load(int id)
      {
         ...
         m_tournament = DB.GetDataSet("select * from Tournament where TournamentID=" + m_id.ToString());
         ...
         System.Data.DataSet rounds = DB.GetDataSet("select * from courses where tournamentid = " + m_id.ToString());
         if (rounds != null)
         {
         ...
         if ( rounds != null )
         {
            ...
            {
               System.Data.DataSet tees = DB.GetDataSet("select * from teedetails where courseid = " + rounds.Tables[0].Rows[x]["courseid"].ToString());
      ...
      public void Save()
      {
         if ( m_id > -1 )
         {
            string sql = "select * from Tournament where TournamentID=" + m_id.ToString();
            System.Data.DataSet dst = DB.GetDataSet(sql);
            if (dst.Tables[0].Rows.Count > 0)
            {
               sql = "update Tournament set Location='', slogan='" + m_tourneyname.Replace("'", "''") +
                  "', description='' where tournamentid=" + m_id.ToString();
            }
            else
            {
               sql = "insert into Tournament (TournamentID, Location, Slogan, Description) values (" + 

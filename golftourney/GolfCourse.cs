...
namespace MonsterGolf
{
	...
	public class GolfCourse
	{
      ...
      {
          DataSet ds = DB.GetDataSet("select * from teedetails where courseid = " + ID +
      ...
      private void Load(System.Data.DataRow course) 
      {
         ...
         DataSet ds = DB.GetDataSet("select * from teedetails where courseid = " + course["courseid"].ToString() +
      ...
      public void Save(int tournamentID)
      {
         ...
         if (this.ID < 0)
         {
            DataSet ds = DB.GetDataSet("select max(courseid)+1 as courseid from courses");
            if (ds.Tables[0].Rows[0]["courseid"] == DBNull.Value)
               this.ID = 1;
            else
               this.ID = int.Parse(ds.Tables[0].Rows[0]["courseid"].ToString());
            DB.Execute("insert into courses (courseid, tournamentid, round, course) values (" +
               this.ID.ToString() + ", " + tournamentID + ", '" + rounds + "', '" + m_coursename.Replace("'", "''") + "')");
         }
         else
         {
            DB.Execute("update courses set course = '" + m_coursename.Replace("'", "''")  + 
         ...
         {
            string sql = "";
            DataSet ds = DB.GetDataSet("select * from teedetails where courseid = " + this.ID.ToString() + 
               " and teenumber = " + x.ToString());
            if (ds.Tables[0].Rows.Count == 0)
            {
               sql = "insert into teedetails (courseid, teenumber, slope, rating) values (" + 
                  this.ID.ToString() + ", " + x.ToString() + ", " + m_teeslope[x].ToString() + ", " + m_teerating[x].ToString() + ")";
               DB.Execute(sql);
            }

            sql = "update teedetails set slope=" + m_teeslope[x].ToString() + ", rating=" + m_teerating[x].ToString();
